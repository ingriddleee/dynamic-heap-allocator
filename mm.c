...
#define NUM_BUCKETS (16) //size of segregated list
#define LOG2(i) 31 - __builtin_clz(i) //function for selecting buckets

/* Global variables */
static block_t *prologue; /* pointer to first block */
static block_t **segrList; //pointer to array of segregated Lists

/* function prototypes for internal helper routines */
static block_t *extend_heap(size_t words, bool toCoalesce);
...
int mm_init(void) {
    ...
    //allocate where I will store my list before allocating the actual heap
    segrList = mem_sbrk(8*NUM_BUCKETS);
    ...
    //put initial block into end of list
    segrList[NUM_BUCKETS-1] = init_block;
...
void *mm_malloc(size_t size) {
    ...
    //If small, much more efficient to just extend rather than search
    if (asize <= 96) {
        extendsize = asize;
...
static block_t *find_fit(size_t asize) {
    /* first fit search */
    block_t * b;
    int i = calcIndex(asize);

    for (i; i < NUM_BUCKETS; i++) {
        b = segrList[i];
        while (b != NULL) {
...
static int calcIndex(int size) {
    ...
    //int num = 64 + LOG2(size);
    int num = LOG2(size) - (16 - NUM_BUCKETS);
    return (num < NUM_BUCKETS-1 ? num : NUM_BUCKETS-1);
}

static void addBlock(block_t * block) {
    int index = calcIndex(block->block_size);

    if(segrList[index] == NULL){
        segrList[index] = block;
        block->body.next = NULL;
        block->body.prev = NULL;
    }
    else {
        block->body.prev = NULL; 
        block->body.next = segrList[index];
        segrList[index]->body.prev = block;
        segrList[index]= block;
    }
}
...
static void removeBlock(block_t * block) {
    ...
    int index = calcIndex(block->block_size);
    block_t * headBlock = segrList[index];

    if (headBlock->body.prev == NULL && headBlock->body.next == NULL) {
        segrList[index] = NULL;
        return;
    }
    //if is head
    if (block == headBlock) {
      segrList[index] = block->body.next;
